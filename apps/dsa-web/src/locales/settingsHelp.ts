...
const settingsHelpZhCN: SettingsHelpMap = {
  ...
  'settings.data_source.ALPHASIFT_ENABLED': {
    ...
    valueNotes: ['自动安装会在 DSA 后端 Python 环境执行；普通 Web 源码部署需要管理员会话，桌面本地模式可直接执行。', '选股结果仅用于研究辅助，不构成投资建议。'],
...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.data_source.ALPHASIFT_ENABLED': {
    ...
    valueNotes: ['Auto-install runs in the DSA backend Python environment; normal Web source deployments need an admin session, while local desktop mode can run it directly.', 'Screening output is for research support only and is not investment advice.'],
