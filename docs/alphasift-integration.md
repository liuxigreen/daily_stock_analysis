...
- 快照源：DSA 内嵌调用默认设置 `SNAPSHOT_SOURCE_PRIORITY=em_datacenter,efinance,akshare_em,tushare`，避免 DSA 环境中存在 `TUSHARE_TOKEN` 时 AlphaSift 优先走 Tushare 并反复产生非致命降级提示；如需恢复 Tushare 优先，可显式覆盖 `SNAPSHOT_SOURCE_PRIORITY`。
...
- `/api/v1/alphasift/strategies`：读取 AlphaSift 策略列表；如果 `ALPHASIFT_ENABLED=true` 但适配层缺失，会先按 `/install` 的同一鉴权要求自动安装后再读取。
- `/api/v1/alphasift/screen`：调用适配层 `screen(..., use_llm=True)`，并在调用期间临时注入 DSA 已解析的 LLM 运行环境，同时向支持 `context` 的适配层传入结构化 LLM 配置；如果已开启但适配层缺失，会先按 `/install` 的同一鉴权要求自动安装后再运行。
